...
const  signUp = async (req,res)=>{
    ...
    try{
        const user = await User.create({
          ...
          const EMAIL_JWT_TOKEN = jwt.sign({username,email}, CONFIG.JWT_EMAIL_SECRET, {
              expiresIn: "10m"
          })
...
const signIn = async (req, res)=>{
    ...
    const pwdCompare = bcrypt.compare(password, authUser.password, function(err, result){
        if(err){
...
const validateEmail = async (req,res)=>{
    try{
        ...
        const update = await User.findOneAndUpdate({username:username}, {confirmed: true});
        console.log(update);
        res.json({
            error: false,
            message: "Your email has been verified successfully. Now you can sign in."
        })
    } catch(err){
        console.log(err);
        res.json({
